        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalLanguages = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalLanguages">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Language</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="LanguageId" type="text" placeholder="Enter LanguageId" v-model="languageToAdd.languageId"></base-input>
  <base-input label="Language" type="text" placeholder="Enter Language" v-model="languageToAdd.language"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="languages" :row-key="record => record.LanguageId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <LanguagePictureView :languages="languages" />
          </div>
...
import LanguagePictureView from './LanguagePictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'languageId',
          visible: true,
          scopedSlots: { customRender: 'languageId' },
          sorter: true
          //sorter: (a, b) => a.languageId - b.languageId,
          //sorter: (a, b) => a.languageId.localeCompare(b.languageId),
        },
        {
          title: 'Language',
		dataIndex: 'language',
      ...
      sortBy: 'languageId',           // Column to sort by
  ...
  methods: {
    ...
    async renderLanguagesTable() {
      ...
      let languagesTableData = [];
      for (let i = 0; i < this.languages.length; i++) {
        languagesTableData.push({
          id: i,
          languageId: this.languages[i].languageId,
          year: this.languages[i].year,
          date: this.languages[i].date,
          competitionId: this.languages[i].competitionId,
          languageId: this.languages[i].languageId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-languages',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForLanguagesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.languageToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.languageToAdd);
      console.log(jsonData);
      
      const res = await LanguageService.addLanguage(jsonData);
