        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalPropertyImages = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPropertyImages">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add PropertyImage</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="PropertyImageId" type="text" placeholder="Enter PropertyImageId" v-model="propertyImageToAdd.propertyImageId"></base-input>
  <base-input label="ImageUrl" type="text" placeholder="Enter ImageUrl" v-model="propertyImageToAdd.imageUrl"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="propertyImageToAdd.description"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="propertyImages" :row-key="record => record.PropertyImageId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PropertyImagePictureView :propertyImages="propertyImages" />
          </div>
...
import PropertyImagePictureView from './PropertyImagePictureView.vue';


const propertyImagesColumns = [
  "propertyImageId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    PropertyImagePictureView
  },

  data() {
    return {
      modalPropertyImages: false,
        isTableView: true,

      columns: [
        {
          title: 'Property Image Id',
		dataIndex: 'propertyImageId',
          visible: true,
          scopedSlots: { customRender: 'propertyImageId' },
          sorter: true
          //sorter: (a, b) => a.propertyImageId - b.propertyImageId,
          //sorter: (a, b) => a.propertyImageId.localeCompare(b.propertyImageId),
        },
        {
          title: 'Image Url',
		dataIndex: 'imageUrl',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} propertyImages`,
      },
      ...
      sortBy: 'propertyImageId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPropertyImagesTable() {
      ...
      let propertyImagesTableData = [];
      for (let i = 0; i < this.propertyImages.length; i++) {
        propertyImagesTableData.push({
          id: i,
          propertyImageId: this.propertyImages[i].propertyImageId,
          year: this.propertyImages[i].year,
          date: this.propertyImages[i].date,
          competitionId: this.propertyImages[i].competitionId,
          propertyImageId: this.propertyImages[i].propertyImageId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-property-images',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPropertyImagesChanged', this.searchQuery);
		//this.renderPropertyImagesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalPropertyImages = false;

      const currentDate = new Date().getTime();
      this.propertyImageToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.propertyImageToAdd);
      console.log(jsonData);
      
      const res = await PropertyImageService.addPropertyImage(jsonData);
