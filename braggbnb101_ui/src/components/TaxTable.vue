        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Taxs</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalTaxs = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalTaxs">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Tax</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="TaxId" type="text" placeholder="Enter TaxId" v-model="taxToAdd.taxId"></base-input>
  <base-input label="TaxAmount" type="text" placeholder="Enter TaxAmount" v-model="taxToAdd.taxAmount"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="taxs" :row-key="record => record.TaxId" :pagination="pagination"
...
import TaxPictureView from './TaxPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'taxId',
          visible: true,
          scopedSlots: { customRender: 'taxId' },
          sorter: true
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} taxs`,
      },
      ...
      sortBy: 'taxId',           // Column to sort by
  ...
  methods: {
    ...
    async renderTaxsTable() {
      ...
      let taxsTableData = [];
      for (let i = 0; i < this.taxs.length; i++) {
        taxsTableData.push({
          id: i,
          taxId: this.taxs[i].taxId,
          year: this.taxs[i].year,
          date: this.taxs[i].date,
          competitionId: this.taxs[i].competitionId,
          taxId: this.taxs[i].taxId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-taxs',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForTaxsChanged', this.searchQuery);
		//this.renderTaxsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalTaxs = false;

      const currentDate = new Date().getTime();
      this.taxToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.taxToAdd);
      console.log(jsonData);
      
      const res = await TaxService.addTax(jsonData);
